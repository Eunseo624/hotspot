...
def main():
    ...
    output_path = "result_corr/pearson_local.csv"
    ...
    for node_id, neighbors in enumerate(ind_list):
        ...
        for col in feature_cols:
            try:
                corr, p_val = pearsonr(neighbor_df[col], neighbor_df[target_col])
                row_result[col] = corr
            except Exception:
    ...
    print(f"\n[완료] 로컬 Pearson 상관계수 결과 저장: {output_path}")
    ...
    for node_id, row in result_df_indexed.iterrows():
        abs_vals = row.abs()
        mean_abs_corr = abs_vals.mean()               # 절댓값 평균
        top_feature = abs_vals.idxmax()               # 가장 큰 피처 이름
        top_corr = row[top_feature]                   # 그 피처의 실제 값
        summary.append([node_id, mean_abs_corr, top_feature, top_corr])

    summary_df = pd.DataFrame(summary,
                              columns=["node_id", "mean_abs_corr", "top_feature", "top_corr"])

    summary_path = "result_corr/pearson_local_summary.csv"
    ...
    plt.title("Pearson Correlation Heatmap (sample)")
    plt.xlabel("Features")
    plt.ylabel("Node ID")
    plt.tight_layout()

    # 저장
    plt.savefig("result/pearson_heatmap_sample.png", dpi=300)
    print("히트맵 저장: result/pearson_heatmap_sample.png")
