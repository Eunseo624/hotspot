...
def main():
    ...
    output_path = "result_corr/spearman_local.csv"
    ...
    for node_id, neighbors in enumerate(ind_list):
        ...
        for col in feature_cols:
            try:
                corr, p_val = spearmanr(neighbor_df[col], neighbor_df[target_col])
                row_result[col] = corr
            except Exception:
    ...
    print(f"\n[완료] 로컬 Spearman 상관계수 결과 저장: {output_path}")
    ...
    summary_path = "result_corr/spearman_local_summary.csv"
    ...
    # 저장
    plt.savefig("result/spearman_heatmap_sample.png", dpi=300)
    print("히트맵 저장: result/spearman_heatmap_sample.png")
